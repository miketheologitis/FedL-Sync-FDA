...
# MODIFIED FOR DASK: from `shuffle_size` -> `2*batch_size`, removed `.prefetch(tf.data.AUTOTUNE)`
def prepare_federated_data_for_test(federated_data, batch_size, num_steps_until_rtc_check, seed=None):
    ...
    def process_client_dataset(client_dataset, batch_size, num_steps_until_rtc_check, seed, shuffle_size=512):
        return client_dataset.shuffle(2*batch_size, seed=seed).repeat().batch(batch_size)\
    ...
    while epoch_count <= num_epochs:
        ...
        while est_var <= theta:
            ...
            if tmp_fda_steps >= fda_steps_in_one_epoch:
                ...
                #gc.collect()  # Force gc every epoch. We need to safeguard for Dask worker's low mem contraints.
