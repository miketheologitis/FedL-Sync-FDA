...
def count_weights2(model):
    """
    Count the total number of trainable and non-trainable parameters in a Keras model.
    ...
    Returns:
    - total_params (int): The total number of trainable parameters and non-trainable in the model.
    """
    total_params = 0
    for layer in model.layers:
        total_params += np.sum([np.prod(weight.shape) for weight in layer.trainable_weights])
...
def average_client_weights2(client_models):
    """
    Compute the average of the trainable and non-trainable parameters across multiple client models.

    This function takes a list of client models and calculates the average of their
    trainable parameters and non-trainable. The averaging is done layer-wise, meaning that the average
    for each layer is computed separately and then returned as a list of average weights
    for each layer.
    ...
    Args:
    - client_models (list of objects): A list of objects representing the client models.
      Each object is expected to have an attribute `trainable_variables` and `non_trainable_variables` that returns
      lists of `tf.Variable` objects representing the trainable and non-trainable parameters of the model.

    Returns:
    - avg_trainable_weights, avg_non_trainable_weights (list of tf.Tensor): A list of tensors representing the
        average weights of the trainable parameters and non-trainable parameters of the client models.
        Each tensor in the list corresponds to the average weight for a specific layer.
    """
    # Retrieve the trainable variables from each client model
...
def synchronize_clients2(server_model, client_models):
    """
    Synchronize the trainable and non-trainable parameters of client models with those of a server model.

    This function takes a server model and a list of client models, and sets the trainable and non-trainable
    parameters of each client model to be equal to those of the server model. This effectively
    synchronizes the client models with the server model.

    Args:
    - server_model (object): An object representing the server model. It is expected to have an
      attribute `trainable_variables` and `non_trainable_variables` that return lists of `tf.Variable` objects
      representing the trainable and non-trainable parameters of the model.
    - client_models (list of objects): A list of objects representing the client models. Each object
      is expected to have a method `set_trainable_variables` and `set_non_trainable_variables` that allows setting
      the trainable and non-trainable variables of the model.
    """
    for client_model in client_models:
        client_model.set_trainable_variables(server_model.trainable_variables)
