...
def prepare_for_federated_simulation(num_clients, train_dataset, batch_size, num_steps_until_rtc_check, seed=None):
    
    # 1. Helper variable to count Epochs
    #fda_steps_in_one_epoch = ((n_train / batch_size) / num_clients) / num_steps_until_rtc_check
    fda_steps_in_one_epoch = 5 # remove
...
def basic_test(num_clients, train_dataset, test_dataset, batch_size, num_steps_until_rtc_check, theta, num_epochs, ams_sketch, epsilon):
    ...
    for fda_name in ["naive", "linear", "sketch"]:
        ...
        server_cnn, client_cnns, federated_dataset, fda_steps_in_one_epoch = prepare_for_federated_simulation(
            num_clients, train_dataset, batch_size, num_steps_until_rtc_check
        )
...
def run_tests(train_dataset, test_dataset, num_clients_list, batch_size_list, num_steps_until_rtc_check_list, theta_list, num_epochs, sketch_width, sketch_depth):
            ...
            for num_steps_until_rtc_check in num_steps_until_rtc_check_list:
                for theta in theta_list:
                    ...
                    complete_epoch_metrics, complete_round_metrics = basic_test(
                        num_clients, train_dataset, test_dataset, batch_size, num_steps_until_rtc_check, theta, num_epochs, ams_sketch, epsilon
                    )
